...
public interface PlanConsumoRepository extends JpaRepository<PlanConsumo, Integer> {
     ...
     @Modifying
     @Transactional
     @Query(value = "INSERT INTO planconsumo (nombre, descuento) VALUES (:nombre, :descuento)", nativeQuery = true)
     void insertarPlanConsumo(@Param("nombre") String nombre, 
     @Param("descuento") Float descuento);
 
     //actualizar planes de consumo
     @Modifying
     @Transactional
     @Query(value = "UPDATE planconsumo SET nombre = :nombre, descuento = :descuento WHERE id = :id", nativeQuery = true)
